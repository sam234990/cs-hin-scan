...
int MuTree::query_tree(double e_threshold, vector<vector<int>> &res, vector<int> &core_id)
{
    ...
    for (int i = 0; i < CoreThresholdMu.size(); i++)
    {
        ...
        { // vertices non contained in any cluster is -1
            core_cnum_id[i] = -1;
        }
    }
